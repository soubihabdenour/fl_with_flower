...
run_experiment() {
  ...
  # Run the experiment
  python -m fedavg_vgg16.main hydra.run.dir=$output_dir dataset.subset="$dataset" dataset.partitioner.name="$partitioner" dataset.partitioner.$partition_param="$partition_value" model.num_classes="$num_classes"
